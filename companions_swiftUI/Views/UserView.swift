    ...
    var body: some View {
        ScrollView{
            ...
            VStack{
                ...
                ExperiseInformationView(user: user)
            }
        }
    }
}

struct ExperiseInformationView: View{
    let user: UserFull
...
struct PersonInformationView: View{
    ...
    init(user: UserFull){
        ...
        self.cursus = self.user.cursus!.cursus.name
        self.member = self.user.member
        map = ["wallet":wallet, "evalPoints":evalPoints, "cursus":cursus, "member":member]
    }
